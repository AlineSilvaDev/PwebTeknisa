function preRender() { //toda vez que atualizar a página a preRender executa
...
function getCountVisibleCards() {
    return Array.from(document.getElementsByClassName("card")).filter(
      (card) =>
        !card.getElementsByClassName.display || //não existe
        card.getElementsByClassName.display !== "none" //diferente de vazio
    ).length;
  } 
  // Array.from => separa o array em letras
  // dentro de filter há uma função filter(função X); ele retorna os resultados que satisfaçam
  //os parametros dentro do filter; o length conta a tamanho.

  
function updateResults(count) { // coloca o numero de resultados onde o id é countResult
    document.getElementById("countResult").textContent = count;
  }
  
function filter() {
    let { search, operation, languages } = getFilterProperties();
    let interval = setInterval((_) => { // arrow function
...
function getFilterProperties() {
    ...
    let languages = Array.from(getSelectedLanguages()).map((lang) => lang.name);
                        //recebe as linguagens que foram selecionadas pela pessoa
    return {
      search,
      operation,
      languages,
  ...
  function getSearchValue() { //retorna o valor do input que foi colocado para procura por nome
    let inputSearchEl = document.getElementById("nameSearch");
    return inputSearchEl.value;
  }
  
  function getSelectedRadio() {//retorna qual 'radio' (E ou OU) está selecionado
    return Array.from(
      document.querySelectorAll('header input[type="radio"]:checked')
    );
  }
  
  function getSelectedLanguages() {// onde o checkbox está marcado (quais linguagens)
    return Array.from(
